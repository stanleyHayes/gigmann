...
> **Partial (2026-07-10 re-assessment):** the three *sweeps* below are **not** implemented and are owner-blocked, not code-blocked — see the progress log. In short: pre-warm can't be done from a separate cron process because the brief cache is in-process (`app.CachedBrief`, already warmed on API boot); it needs shared Redis. The stalled-action/licence-expiry sweeps' real value is *proactive push* (owner VAPID keys) plus an alert-dedup mechanism for idempotent daily re-runs — and licence risk is already surfaced in the Daily Brief by the signal engine. Both also need product-owned staleness-threshold policy. Deliberately not invented unattended.
...
- Acceptance criteria:
  - [ ] Cron-style worker: morning pre-warm, stalled-action sweep, licence-expiry sweep. *(migrate + refresh-views shipped; the three sweeps are owner-blocked — see the 2026-07-10 note above.)*
...
| 2026-06-30 | **Production-readiness pass #1 — config fail-fast + deploy blueprint (from a 5-agent spec audit).** `config.validate()` now rejects a too-short `JWT_SECRET` (<32 chars) and a wildcard `CORS_ALLOWED_ORIGINS` outside development, and bootstrap logs a WARN when running on ephemeral in-memory repos outside dev (the DB-less demo posture is intentional but no longer silent). `render.yaml` gains `TRUST_PROXY=true` (correct client IP behind Render's proxy → real per-IP rate limiting), documents the VAPID/Sentry/OTel secret keys, and makes the Postgres+Redis+refresh-cron persistence path one-uncomment-ready. `backend/.env.example` rewritten to exactly the 21 vars the code reads (dropped stale JWT-key-path/TTL/HTTP-timeout vars; added TRUST_PROXY/SENTRY/VAPID/OTEL/FEATURE_*); README env docs synced. Tests added for the new validations. Verified: lint 0, cover gate 85.2%. | Claude |
| 2026-06-30 | **CI: run the k6 load test (GEC-102) in CI.** The Daily Brief hot-path load test (`infra/load/brief-load.js`, p95<500ms) was previously only reachable via `make load-test`. Added `.github/workflows/load-test.yml` (weekly schedule + `workflow_dispatch`, not per-push to keep pushes green): boots the API on the in-memory demo network, installs a pinned k6, and runs the test. Safe because `setup()` logs in once and the load is on the cached `/brief`+`/metrics` GETs. (Tightening the a11y→0.95 / SEO Lighthouse gates and frontend branch-coverage→80 needs the underlying SSG + more tests first — flagged, not forced.) | Claude |
| 2026-07-10 | **Frontend coverage restored — un-reds `main`.** The `d347d81` cockpit overhaul (AppShell +962 lines, new AttentionFeed/PaginationControls/PageHeader/SurfaceCard/EmptyState, screen rewrites) shipped under-tested, dropping all four coverage metrics below the gate (functions 69%). Added/extended tests: a full interaction-driven `AppShell.test.tsx` (theme toggle, notifications, help, sidebar collapse, account menu + sign-out, nav, mobile drawer), `AttentionFeed.test.tsx` (task/resolve/dismiss + loading/error/empty), `LoginScreen.test.tsx` (sign-in, password-visibility, reset-request), `providers.test.tsx` (colour-mode setMode/toggle/setPreset), `helpEvents.test.ts`, a route-chunk loader test (resolves every lazy screen import), and interaction tests for FacilityDetail (task/draft/copy) + AskScreen (copy draft). Frontend: 144 tests; statements 85.3% / branches 74.9% / functions 81.8% / lines 85.5% — all above the gate. Also fixed my own `4e80bce` secret-scan slip (gitleaks allowlist for the `config_test.go` JWT fixtures) and the backend govulncheck stdlib vuln (`check-latest` → go1.25.12). | Claude |
| 2026-07-10 | **E2E gate un-redded (chronic failure since the visibility toggle shipped).** The Playwright demo journey's `getByLabel(/password/i)` began matching two elements — the password input *and* the new sign-in reveal button (`aria-label "Show sign-in password"`) — a strict-mode violation that re-failed the `E2E` workflow on *every* push across multiple agents. Matched the input's exact `"Password"` aria-label. Verified locally end-to-end against a freshly-booted in-memory stack (login → brief → network → ask → my day → approvals, 1 passed). | Claude |
| 2026-07-10 | **a11y + SEO polish (audit follow-ups).** Added a keyboard "Skip to main content" link (WCAG 2.4.1) → `<main id="main-content">`, honouring `prefers-reduced-motion`; replaced the notifications-dropdown loading text with skeleton loaders and the Reports "Generating PDF…" button text with `ButtonLoadingDots` (project loader rules). Added Open Graph / Twitter-Card / JSON-LD share metadata to the public `privacy.html` and `terms.html` (welcome.html already had it). Frontend: 144 tests, typecheck + eslint clean, all coverage gates green. | Claude |
| 2026-07-10 | **GEC-71 sweeps re-assessed — honestly *not* done (owner-blocked, not implemented).** The story header is ticked but its acceptance criterion (morning pre-warm + stalled-action + licence-expiry sweeps) is not: (1) the brief cache is **in-process** (`app.CachedBrief`, warmed on API boot — see the "brief cache warmed" startup log), so a *separate* cron worker cannot pre-warm the API process; a cross-process pre-warm needs shared Redis (owner infra). (2) staff-licence data is seeded into the signal engine, **not** behind a repository port, and licence risk is *already* surfaced in the Daily Brief by the signal engine. (3) both sweeps' real value is *proactive push*, which needs owner VAPID keys, plus an alert-dedup mechanism (new schema) for idempotent daily re-runs. Implementing them unattended would mean inventing staleness-threshold policy and adding infra/schema that belong to the owner — flagged here rather than forced. Criterion left unchecked with this rationale. | Claude |
| 2026-07-10 | **GEC-105/107 — split hosting: SPA → Vercel, API + Postgres → Render.** The SPA now deploys to Vercel at `https://gigmann.vercel.app`; the `gigmann-frontend` Render static service is **deleted** (it would double-deploy and is CORS-blocked by the new allow-list). Added [`frontend/vercel.json`](frontend/vercel.json): the SPA rewrite (without it react-router deep links 404) plus the security headers ported from the old Render static config — schema checked against Vercel's live docs. Added `frontend/.env{,.production,.example}` documenting the only two build-time vars (`VITE_API_BASE_URL` required — it falls back to `/`, which on Vercel would send every API call to a backend-less origin; `VITE_SENTRY_DSN` optional). `VITE_*` values are inlined into the bundle and therefore **public** — verified by building and grepping `dist/assets/*.js`. Repointed every published URL (canonical/OG/Twitter, `sitemap.xml`, `robots.txt`, `security.txt`) off `gigmann-frontend.onrender.com`; because those URLs live *inside* the inline JSON-LD, all three CSP `sha256` hashes were recomputed and re-verified against the built `dist/` output. API `CORS_ALLOWED_ORIGINS` → the Vercel origin (note: Vercel preview URLs are dynamic and cannot be covered by a static allow-list). | Claude |
| 2026-07-10 | **GEC-11/12/13 — Postgres actually provisioned (was only ever commented out).** `infra/render.yaml` now declares the `gigmann-postgres` database (**`plan: basic-256mb`, paid on purpose — Render deletes `free` Postgres after 30 days**, PG 18, db/user `gigmann`) and injects `DATABASE_URL` into the API via `fromDatabase`/`connectionString`. No manual migrate step is needed: the API runs the embedded advisory-locked migrations on boot (`internal/bootstrap`), which create the pgvector extension and seed the synthetic network on first run — previously the deployment silently ran on **ephemeral in-memory repos**. Added an `ipAllowList` entry so the database is reachable from outside Render (use the dashboard's *External* connection string; the injected one is internal). ⚠️ It is `0.0.0.0/0` — narrow it to `<your.ip>/32` once known. Redis and the MV-refresh cron stay commented (optional; in-process fallbacks are correct on one instance). Verified: render.yaml parses; frontend 144 tests, typecheck + lint clean, all coverage gates green. | Claude |
| 2026-07-24 | **CI un-redded — three newly-disclosed advisories (not a code regression).** `main` went red across three jobs on a docs/infra-only commit; all three were CVEs published after the previous green run, not the change. Fixed each at its source: `golang.org/x/text` v0.37.0 → **v0.39.0** (GO-2026-5970, infinite loop on invalid input — reached via `pgx.ConnectConfig` in `postgres.Migrate`, so genuinely in a live path); `google.golang.org/grpc` v1.81.1 → **v1.82.1** (GHSA-hrxh-6v49-42gf HIGH, flagged by Trivy in the `api` gobinary via the OTLP exporter — a *different* advisory from the govulncheck one, easy to mistake for a duplicate); and `dompurify` 3.4.11 → **3.4.12** (GHSA-c2j3-45gr-mqc4, `npm audit --omit=dev`; transitive via jspdf, lockfile-only). Also set the Render Postgres + web service to the **free** plans per owner decision, recording the tradeoffs in the Blueprint (free Postgres is deleted ~30 days after creation; free web services cold-start after ~15 min idle). Removed `DATABASE_URL`/`REDIS_URL` from `backend/.env.production` — Render injects the former via `fromDatabase`, so a hand-set empty value could only override the real one and silently drop the API to in-memory repos. Verified locally: backend build/vet clean, **govulncheck 0 vulnerabilities**, coverage 85.2%; frontend typecheck/lint/build clean, 144 tests, all coverage gates green, `npm audit --omit=dev` 0. | Claude |
| 2026-07-24 | **Follow-up: `go.mod` toolchain drift broke the container build (self-inflicted, caught in CI).** The CVE bump above was run with a local Go 1.26.5, and `go mod tidy` silently rewrote the directive `go 1.25.0` → `go 1.26.5`. `backend/Dockerfile` builds on `golang:1.25-alpine`, which sets `GOTOOLCHAIN=local` and therefore cannot auto-upgrade, so `RUN go mod download` failed and the Trivy job never reached the scan. **It passed everywhere else** — local `go build` and all CI Go jobs default to `GOTOOLCHAIN=auto` and quietly download the newer toolchain, so the container was the only place the constraint was actually enforced. Checked `go list -m -f '{{.Path}} {{.GoVersion}}' all`: every dependency declares `go 1.25.0` and only this module declared 1.26.5, so the bump was pure toolchain drift — restored `go 1.25.0` rather than raising the Dockerfile/CI to 1.26, which would have contradicted the deliberate `go-version: 1.25.x` pinning (see the ci.yml comments on codegen drift). Verified by reproducing the container builder exactly: `GOTOOLCHAIN=go1.25.12 go mod download && go build ./...` both pass. **Lesson for agents: after `go get`/`go mod tidy`, always `git diff backend/go.mod` and revert an unintended `go` directive bump.** | Claude |
| 2026-07-24 | **Dependency upgrade reconciled + codegen toolchain unbroken.** Owner staged a broad `go get -u` bump (anthropic-sdk 1.60, pgx 5.10, kin-openapi 0.144, crypto 0.54, oapi-codegen 2.8.0, …). `go mod tidy` (pinned `GOTOOLCHAIN=go1.25.12` to hold the `go 1.25.0` directive) reconciled it, but the blanket `-u` also pulled `dprotaso/go-yit` to a yaml.v4 build incompatible with the pinned `vmware-labs/yaml-jsonpath v0.3.2` (yaml.v3) — breaking `make generate` (the oapi-codegen tool) and CI's codegen-drift job, even though the app itself builds/tests clean (it never imports that chain). `go mod graph` showed nothing actually required the new go-yit, so pinned it back to the main version; `make generate` then compiled and regenerated `openapi_gen.go` (v2.7.1→v2.8.0 output — cosmetic doc-comment prefixes only). Verified end-to-end under Go 1.25.12. | Claude |
| 2026-07-24 | **Seed demo accounts for both roles (was executive-only in practice).** `demoAccounts` now builds 14 identities — 2 executives (network-wide) + a facility manager per facility (names mirror the seeded managers), all sharing `DEMO_PASSWORD`. Added `reconcileDemoAccounts`: because `postgres.EnsureSeeded` only seeds an EMPTY database, new identities would never reach the already-seeded production store — this additively upserts any account missing by email on every boot, never clobbering an existing one (so a changed password or enrolled MFA is preserved). No-op on a fresh in-memory store. Verified: build/vet/gofmt/lint(0)/govulncheck(0)/cover 85.2%, and live login against an in-memory boot — executives have no facility, managers are correctly facility-scoped (`tafo.manager` → `tafo-maternity`), existing `ceo@` still works, wrong password → 401. Team login sheet lives in the gitignored `DEMO-ACCESS.md`. | Claude |
